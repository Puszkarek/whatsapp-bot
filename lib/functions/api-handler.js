...
module.exports = {
    ...
    async animequote(callback, {
        argumentsTxt
    }) {
        let url = configURLParams("https://animechan.vercel.app/api/quotes/anime?", {
            title: argumentsTxt
        });
        let _response = "";
    ...
    async waifu(callback) {
        let callbackResponse = {}
        requestFrom('https://mhankbarbar.herokuapp.com/api/waifu')
            .then(async (response) => {
                const waifu = response;
                const _caption = `❤️ Name : ${waifu.name}\n🎉️ Description : ${waifu.desc}\n\n❇️ Source : ${waifu.source}`;
                const imgUrl = waifu.image;
                const img = await fetch(imgUrl);
                const buffer = await img.buffer();
                const _type = imageType(buffer);
                const _imgURI = `data:${_type.mime};base64,${buffer.toString("base64")}`
                const _imgFiletype = "file." + _type.ext;
                callbackResponse = {
    ...
    async covid(callback, {
        argumentsTxt
    }) {
        const callbackResponse = {}
        callbackResponse.type = "reply"
        let countryToSearch = OPTIONS.country
        if (argumentsTxt.length > 0) {
            countryToSearch = argumentsTxt;
        }
        requestFrom('https://coronavirus-19-api.herokuapp.com/countries/' + countryToSearch)
            .then(async (response) => {
                let _response = ""
                if (response != "Country not found") _response = RESPONSES.covid(response);
                else _response = RESPONSES.error.invalid;
                callbackResponse.response = _response;
                callback(callbackResponse);
            })
            .catch((err) => {
    ...
    async weather(callback, {
        ...
        let callbackResponse = {}
        let url = configURLParams("http://api.weatherapi.com/v1/current.json?aqi=yes&", {
            key: process.env.WEATHER_API_KEY,
            q: argumentsTxt
        });
        requestFrom(url)
            .then((response) => {
                _response = weatherResponse(response);
                callbackResponse = {
                    type: 'reply',
                    response: _response
                }
    ...
    async vagalume(callback, {
        ...
        try {
            ...
            let url = configURLParams("https://api.vagalume.com.br/search.php?", {
            ...
            requestFrom(url)
                .then((response) => {
                    if (response.type != "song_notfound" && response.type != "notfound") {
                        let _response = response.art.name + "\n\n";
                        _response += response.mus[0].name + "\n\n";
                        if (searchTerms[2] != null && response.mus[0].translate != null) {
    ...
    async horoscope(callback, {
        argumentsTxt
    }) {
        let url = "http://horoscope-api.herokuapp.com/horoscope/today/" + argumentsTxt;
        requestFrom(url)
            .then((response) => {
    ...
    async movie(callback, {
        ...
        let isRandom = false;
        if (argumentsTxt.length > 0) {
            url = "https://api.themoviedb.org/3/search/multi?sort_by=popularity.desc&page=1&include_adult=true&";
            url = configURLParams(url, {
            ...
            url = "https://api.themoviedb.org/3/discover/movie?include_adult=true&";
            url = configURLParams(url, {
                api_key: process.env.MOVIE_API_KEY,
                language: languageId
            })
    ...
    async bill(callback, {
        ...
        if (argumentsTxt.length != 0) {
            argumentsTxt = capitalizeText(argumentsTxt);
            url = configURLParams(url, {
                text: argumentsTxt
            });
        } else {
    ...
    async quote(callback, {
        argumentsTxt
    }) {
        let quotes = await (await quotesSearch({
            term: argumentsTxt,
            max: 10
        })).phrases;
        let _response = "";
        for (let q in quotes) {
            _response += '"' + quotes[q].text.replace(/.(?=[A-Z])/g, "$&\n") + '"' + "\n";
            _response += "-" + quotes[q].author + "\n\n";
        }
        let callbackResponse = {
            type: 'reply',
