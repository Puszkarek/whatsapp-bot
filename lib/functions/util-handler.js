...
let initCalendars = require("../calendar")
const {
    currentMonth,
    calendars
} = initCalendars();
...
module.exports = {
                            ...
                            try {
                                if (!err) {
                                    ...
                                    if (argumentsTxt.includes("[")) {
                                        ...
                                        const params = JSON.parse(argumentsTxt);
                                        scale = params[0];
                                        stickerArguments = params[1];
                                        if (params[2] != null) {
                                            color = params[2]
                                        }
                                    }
                                    ...
                                    let fontStroke = (imageWidth * fontStrokeRatio) * scale;
                                    ...
                                    im(media).font("./lib/theboldfont.otf", fontSize)
                                        ...
                                        .fill(color)
                                        .drawText(0, bottomOffset, stickerCaptions, "South")
                                        .resize(512, 512)
    ...
    async calendar(callback, {
        ...
        try {
            let _response = "";
            const firstArgs = argumentsList[0];
            const secondArgs = argumentsList[1];
            const calendarType = firstArgs != null ? firstArgs : "class";

            const calendarMonth = secondArgs != null ? secondArgs : currentMonth;
            _response = calendars[calendarMonth][calendarType];
            if (_response == undefined) _response = RESPONSES.error.invalid;
            callbackResponse.response = _response;
            callback(callbackResponse)
        } catch (err) {
            callbackResponse.type = 'error';
