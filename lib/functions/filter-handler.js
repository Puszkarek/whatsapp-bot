...
module.exports = {
	async filter(
		callback,
		{ argumentsTxt, argumentsList, isGroupMsg, groupId, quoteTxt },
	) {
		let _response = "";
		...
		if (isGroupMsg) {
			...
			if (filterContent.length > 1) {
				let currentFilter =
					filters[groupId] != null ? filters[groupId] : {};
				currentFilter[key] = filterContent.trim();
				newFilterList = JSON.stringify(
					Object.assign({}, currentFilter),
				);
				fs.writeFileSync(
					getFilterPath(filterPath, groupId),
					newFilterList,
				);
	...
	async removefilter(
		callback,
		{ argumentsTxt, argumentsList, isGroupMsg, groupId },
	) {
		const callbackResponse = {};
		callbackResponse.type = "reply";
		let _response = "";
		if (isGroupMsg) {
			if (argumentsTxt.length > 0) {
				...
				let newFilterList = JSON.stringify(
					Object.assign({}, currentFilter),
				);
				fs.writeFileSync(
					getFilterPath(filterPath, groupId),
	...
	async listfilters(callback, { isGroupMsg, groupId }) {
		const callbackResponse = {};
		callbackResponse.type = "reply";
		let _response = "";
		if (isGroupMsg) {
	...
	async searchFilter(callback, { argumentsTxt, groupId }) {
